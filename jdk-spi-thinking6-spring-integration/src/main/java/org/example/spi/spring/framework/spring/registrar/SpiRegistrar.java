package org.example.spi.spring.framework.spring.registrar;


import cn.hutool.core.map.MapUtil;

import org.apache.commons.lang3.StringUtils;
import org.example.spi.spring.framework.anotation.EnableSpi;
import org.example.spi.spring.framework.anotation.Spi;
import org.example.spi.spring.framework.factory.SpiFactory;
import org.example.spi.spring.framework.factory.SpiFactoryLoader;
	...
	private void registerSingleton(AnnotationMetadata importingClassMetadata) {
        ...
        if (!CollectionUtils.isEmpty(spiInterfaceList)) {
    		for (Class<?> spiInterface : spiInterfaceList) {
    			...
    			try {
    				spiMap = SpiFactoryLoader.getSpiFactory().getSpiMap(spiInterface);
